...
DROP POLICY IF EXISTS "Allow service role read access" ON qr_codes;
DROP POLICY IF EXISTS "Allow service role insert" ON qr_codes;
DROP POLICY IF EXISTS "Allow service role update" ON qr_codes;
DROP POLICY IF EXISTS "Allow service role delete" ON qr_codes;
...
CREATE POLICY "Allow service role insert" ON qr_codes
    ...
    WITH CHECK (
        ...
        email_name IS NOT NULL AND
        memo IS NOT NULL AND
        qr_data IS NOT NULL AND
        amount IS NOT NULL
    );
